	...
	export default {
		...
		methods: {
			...
			async fetchDailyOrdersAndStats() {
				...
				try {
					const res = await uni.cloud.callFunction({
						name: 'order-center',
						data: { action: 'getDailyOrderStats', token: this.token, dateString: this.selectedDate }
					});
			...
			goToOrderDetails(orderId) {
				uni.navigateTo({ url: `/pages/common/orderDetails?id=${orderId}` });
			},
			goToLogin() {
				this.logout();
			}
...
<style scoped>
	.statistics-container { padding: 20rpx; background-color: #f4f4f4; min-height: 100vh; }
	.header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 20rpx; background-color: #fff; padding: 20rpx; border-radius: 12rpx; box-shadow: 0 2rpx 8rpx rgba(0, 0, 0, 0.05); }
	.page-title { font-size: 36rpx; font-weight: bold; color: #333; }
	.date-picker-section { background-color: #fff; padding: 20rpx; border-radius: 12rpx; margin-bottom: 20rpx; display: flex; justify-content: center; box-shadow: 0 2rpx 8rpx rgba(0, 0, 0, 0.05); }
	.picker-display { border: 1px solid #007aff; color: #007aff; padding: 15rpx 30rpx; border-radius: 8rpx; font-size: 30rpx; text-align: center; }
	.arrow { margin-left: 10rpx; }
	.stats-section { background-color: #fff; padding: 25rpx; border-radius: 12rpx; margin-bottom: 20rpx; box-shadow: 0 2rpx 8rpx rgba(0, 0, 0, 0.05); }
	.section-header { font-size: 30rpx; font-weight: bold; color: #333; margin-bottom: 25rpx; padding-bottom: 15rpx; border-bottom: 1rpx solid #eee; }
	.stats-summary-flex-wrap { display: flex; flex-wrap: wrap; gap: 18rpx; }
	.stat-item { display: flex; justify-content: space-between; align-items: center; font-size: 27rpx; padding: 15rpx; border: 1rpx solid #e8e8e8; border-radius: 8rpx; background-color: #f9f9f9; flex-basis: calc(50% - 10rpx); min-width: 280rpx; box-sizing: border-box; }
	.stat-label { color: #555; margin-right: 15rpx; white-space: nowrap; }
	.stat-value { color: #333; font-weight: bold; text-align: right; word-break: break-word; }
	.cost-value { color: #e74c3c; }
	.performance-item { display: flex; justify-content: space-between; align-items: center; font-size: 28rpx; padding: 12rpx 0; border-bottom: 1rpx solid #f0f0f0; }
	.performance-item:last-child { border-bottom: none; }
	.performance-item .name { color: #333; flex-basis: 45%; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
	.performance-item .count { color: #007bff; font-weight: bold; margin-left: 10rpx; }
	.performance-item.supplier-perf-item .details { display: flex; flex-direction: column; align-items: flex-end; margin-left: 10rpx; }
	.performance-item .total-cost { font-size: 24rpx; color: #e74c3c; }
	.order-list-title { font-size: 32rpx; font-weight: bold; margin: 30rpx 0 15rpx; padding-left: 10rpx; color: #333; }
	.order-scroll-view { max-height: 600rpx; background-color: #fff; border-radius: 12rpx; padding: 0 10rpx; box-shadow: 0 2rpx 8rpx rgba(0, 0, 0, 0.05); margin-bottom: 20rpx; }
	.order-card { background-color: #fdfdfd; border: 1px solid #e9e9e9; border-radius: 8rpx; padding: 20rpx; margin: 20rpx 0; }
	.order-row { display: flex; font-size: 26rpx; margin-bottom: 10rpx; line-height: 1.6; }
	.order-label { width: 150rpx; color: #666; flex-shrink: 0; }
	.order-value { flex: 1; color: #333; word-break: break-all; }
	.order-value.important-value { font-weight: 500; color: #007bff; }
	.id-value { color: #555; }
	.date-value { font-size: 24rpx; color: #777; }
	.type-value.cdk { color: #17a2b8; }
	.type-value.gift { color: #fd7e14; }
	...
	.loading-text, .empty-text, .unauthorized-text, .error-text { text-align: center; color: #6c757d; margin-top: 50rpx; font-size: 28rpx; padding: 20rpx; }
	.error-text { color: #e74c3c; background-color: #fdd; border: 1rpx solid #fbb; border-radius: 8rpx; }
