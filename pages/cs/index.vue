  ...
  <view class="cs-dashboard-container">
    <view v-if="isLoggedIn && userRole === 'cs'">
      <view class="header">
        ...
        <button class="action-button" size="mini" @click="goToCreateOrderPage">创建新订单</button>
      </view>
...
export default {
  ...
  methods: {
    ...
    async fetchMyOrders() {
      ...
      if (!currentToken) {
        this.goToLogin(); // Token丢失，理论上 checkCsAuthAndLoadData 已处理
        this.loadingOrders = false;
        return;
      }

      try {
        // 后端 order-center 的 viewOrders action 会根据CS角色自动过滤订单
        ...
        if (res.result.code === 200) {
          ...
          // this.totalOrders = res.result.total; // 如果有分页
    ...
    goToOrderDetails(orderId) {
      // 我们稍后会创建 pages/common/orderDetails.vue 页面
      uni.navigateTo({
...
.order-list-section {
  ...
  padding: 20rpx;
  border-radius: 12rpx;
  box-shadow: 0 2rpx S10rpx rgba(0,0,0,0.03);
}
.list-header {
  display: flex;
  justify-content: space-between;
...
.order-scroll-view {
  height: calc(100vh - 450rpx); /* 根据实际页面布局调整高度 */
}
.order-card {
